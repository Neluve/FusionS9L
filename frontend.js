...
function makeUI()
{
    ...
    italic.appendChild(document.createTextNode('Click en Nombre de Usuario para Generar una ID'));
    ...
    tdBase64.appendChild(document.createTextNode('.: JMC JuanMa :.'));
    ...
    for(var i = 0; i < 16; i++)
    {
        ...
        button.appendChild(document.createTextNode('Poner y Activar'));
...
function fetchAccount(i)
{
    ...
    if(!mailbox.account_name)
    {
        ...
        italic.appendChild(document.createTextNode('Esta kuenta No Existe!'));
        ui[i].id.value = '';
...
function activateAccount(idx)
{
    ...
    for(var i = 7; i >= 0; i--)
    {
        ...
        if(digit != digit)
        {
            alert("Debe tener 16 caracteres y ke solo kontengan [0-9A-F]");
            return;
        }
    ...
    if(ui[idx].id.value == acc_id.toUpperCase())
        alert("ID de PSN Kargada Korrectamente!");
    return false;
}
